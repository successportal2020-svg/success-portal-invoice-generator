...
async function saveRecord(){const data=collect(),error=validate(data);if(error){alert(error);return false}const savedNumbers=JSON.parse(localStorage.getItem('successPortalSavedNumbers')||'[]');if(savedNumbers.includes(data.number)){alert(`${data.type.toUpperCase()} ${data.number} has already been saved. Click New document to create another one.`);$('#saveStatus').textContent=`Already saved: ${data.number}`;return false}const url=DEFAULT_SCRIPT_URL;$('#saveStatus').textContent='Saving…';try{await fetch(url,{method:'POST',mode:'no-cors',cache:'no-store',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(data)});savedNumbers.push(data.number);localStorage.setItem('successPortalSavedNumbers',JSON.stringify(savedNumbers.slice(-500)));localStorage.setItem('successPortalLastRecord',JSON.stringify(data));const savedAt=new Date().toLocaleTimeString();$('#saveStatus').textContent=`Saved ${data.number} at ${savedAt}`;alert(`${data.type.toUpperCase()} ${data.number} was sent to the Google Sheet.`);return true}catch(e){$('#saveStatus').textContent='Save failed';alert('Could not send the record. Check your internet connection and try again.');return false}}
